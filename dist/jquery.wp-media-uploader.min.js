...
{if(self.is_lodash()){_.noConflict();}
...
self.templates.inside=self.$target.html();if(self.$target===undefined)
...
if(self.options.templateFile){$template=$(self.options.templateFile);if($template.length)
self.templates.file=$template.html();}
if(self.options.templateEmbed){$template=$(self.options.templateEmbed);if($template.length)
...
$html.find('.inject-media-url').html(media.url);if($html.find('.inject-media-id').length)
$html.find('.inject-media-id').html(media.id);if($html.find('.inject-media-filename').length)
$html.find('.inject-media-filename').html(media.filename);return $html.get();}
...
media.filename=self.get_filename(media.url);attachments.push(media);}
...
{var ids=values.filter(function(id){return!isNaN(id);}).map(function(id){return id.trim?id.trim():id;});if(wp&&wp.api&&ids.length){self.$el.prop('disabled',true);self.$el.addClass('loading');self.$el.attr('disabled','disabled');wp.apiRequest({path:'wp/v2/media',method:'GET',data:{include:ids,per_page:100,},}).then(self.on_load_attachments);}else if(values.length){var attachments=[];for(var i in values){attachments.push({_model:undefined,id:1,type:'file',mime:undefined,subtype:undefined,url:values[i],filename:self.get_filename(values[i]),});}
...
media.filename=self.get_filename(media.url);attachments.push(media);}
self.render(attachments);self.$el.prop('disabled',false);self.$el.removeAttr('disabled');self.$el.removeClass('loading');};self.get_filename=function(url)
{return url.substring(url.lastIndexOf('/')+1);};self.clear=function()
{self.$target.html('');self.$target.html(self.templates.inside);self.$el.trigger('uploader:clear');};self.is_lodash=function()
{let isLodash=false;if('undefined'!=typeof(_)&&'function'==typeof(_.forEach)){const funcs=['get','set','at','cloneDeep'];isLodash=true;funcs.forEach(function(func){isLodash=('function'!=typeof(_[func]))?false:isLodash;});}
return isLodash;};self.ready();window.media_uploaders[self.$el.attr('id')]=self;return self;};$(document).ready(function(){$('*[role="media-uploader"][data-editor]').each(function(){$(this).wp_media_uploader();});});})(jQuery);