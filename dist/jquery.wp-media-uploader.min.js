...
$html.find('input').attr('name',name+(self.options.multiple?'[]':''));if(!self.options.idValue&&media.url)
...
{var attachments=[];for(var i in models){var media={_model:models[i],id:models[i].id,type:models[i].get('type'),subtype:models[i].get('subtype'),url:models[i].get('url'),};if(models[i].get('alt'))
...
media.img=models[i].get('image').src;attachments.push(media);}
...
{var selection=self.frame.state().get('selection');self.parse_capture(selection.models);self.$el.trigger('uploader:selection',[selection.models,self]);};self.load_attachments=function(ids)
{ids=ids.filter(function(id){return!isNaN(id);}).map(function(id){return id.trim();});if(wp&&wp.api&&ids.length){self.$el.prop('disabled',true);self.$el.addClass('loading');self.$el.attr('disabled','disabled');wp.apiRequest({path:'wp/v2/media',method:'GET',data:{include:ids,per_page:100,},}).then(self.on_load_attachments);}};self.on_load_attachments=function(data)
{var attachments=[];for(var i in data){var media={_model:undefined,id:data[i].id,type:data[i].media_type,subtype:data[i].mime_type.replace('/'+data[i].media_type+'\//g',''),url:data[i].guid.rendered?data[i].guid.rendered:data[i].guid,};if(data[i].alt_text)
...
media.url=data[i].media_details.sizes[self.options.size].source_url;attachments.push(media);}
